...
if len(sys.argv) < 4:
	...
	print '\nSpecify size of EBS volume to be created (in GB) with IOPS. Be careful! Cost structure is different than gp2 EBS volumes. Along with availability zone (e.g. us-east-1b) and description provided in double quotes\n'
	sys.exit()
...
volID=subprocess.Popen('aws ec2 create-volume --size %s --availability-zone %s --volume-type io1 --iops %s  %s | grep VolumeId' %(volSize,AZ,IOPS,snapid),shell=True, stdout=subprocess.PIPE).stdout.read().strip().split()[-1].split('"')[1]
